...
void INS_task(void const *pvParameters)
{
  ...
  //  PID_init(&imu_temp_pid, PID_POSITION, imu_temp_PID, TEMPERATURE_PID_MAX_OUT, TEMPERATURE_PID_MAX_IOUT);
    ...
    while (1)
    {
        ...
        AHRS_update(INS_quat, 0.002f, bmi088_real_data.gyro, bmi088_real_data.accel, ist8310_real_data.mag);
        get_angle(INS_quat, INS_angle + INS_YAW_ADDRESS_OFFSET, INS_angle + INS_PITCH_ADDRESS_OFFSET, INS_angle + INS_ROLL_ADDRESS_OFFSET);
        counter ++;
        if (counter >= 100) {
            sprintf(buffer,"A:%.3f,%.3f,%.3f\r\n", INS_angle[0], INS_angle[1], INS_angle[2]);
...
static void imu_temp_control(fp32 temp)
{
    ...
    if (first_temperate)
    {
        PID_Calculate(&imu_pid, 45.0f, temp);
        if (imu_pid.output < 0.0f)
        {
            imu_pid.output = 0.0f;
        }
        tempPWM = (uint16_t)imu_pid.output;
    ...
    else
    {
        ...
        if (temp > 45.0f)
        {
            ...
            if (temp_constant_time > 200)
            {
                ...
                //
                first_temperate = 1;
                imu_pid.output= MPU6500_TEMP_PWM_MAX / 2.0f;
            }
        }
